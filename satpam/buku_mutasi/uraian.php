                ...
                <?php if ($laporan['status'] === 'draft') { ?>
                    <form method="post">
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-lg-4"><label class="form-label">Nama Satpam</label><div class="form-control activity-info"><i class="bi bi-person text-primary me-2"></i><?= htmlspecialchars($namaSatpam) ?></div></div>
                            <div class="col-lg-4"><label class="form-label">Tanggal &amp; Waktu Input</label><div class="form-control activity-info"><i class="bi bi-calendar3 text-primary me-2"></i><?= $tanggalTampil ?> <span class="ms-1 text-muted">• <?= date('H:i') ?> WIB</span></div></div>
                            <div class="col-lg-4"><label class="form-label">Shift</label><div class="form-control activity-info"><i class="bi bi-clock text-primary me-2"></i><?= htmlspecialchars($laporan['nama_shift']) ?> <span class="ms-1 text-muted">(<?= substr($laporan['jam_mulai'], 0, 5) ?>–<?= substr($laporan['jam_selesai'], 0, 5) ?>)</span></div></div>
                        </div>
            ...
            <div class="card-body">
                ...
                <div class="inventory-table table-responsive">
                    <table class="table align-middle">
                        <thead><tr><th>No.</th><th>Tanggal &amp; Waktu</th><th>Shift</th><th>Uraian Kegiatan</th><th class="text-center">Aksi</th></tr></thead>
                        <tbody>
                        <?php if (!$uraianKegiatan) { ?><tr><td colspan="5" class="text-center text-muted py-5">Belum ada uraian kegiatan yang ditambahkan.</td></tr><?php } ?>
                        <?php foreach ($uraianKegiatan as $index => $row) { ?>
                            <tr>
                                <td><?= $index + 1 ?></td>
                                <td><div><?= $tanggalTampil ?></div><small class="text-muted"><i class="bi bi-clock me-1"></i><?= htmlspecialchars(substr($row['jam'], 0, 5)) ?> WIB</small></td>
...
<?php if ($laporan['status'] === 'draft') { foreach ($uraianKegiatan as $row) { ?>
<div class="modal fade" id="edit<?= (int) $row['id_uraian'] ?>" tabindex="-1" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><form method="post">
    <div class="modal-header"><h5 class="modal-title">Edit Uraian Kegiatan</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
