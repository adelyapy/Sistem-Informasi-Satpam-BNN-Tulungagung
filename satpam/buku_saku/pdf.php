...
$materiTersedia = materiBukuSakuTableExists($conn);
$kategori = false;
...
<main class="public-page">
  <div class="container public-shell">
    ...
    <?php if (mysqli_num_rows($dokumen)): ?>
      <h2 class="public-title fs-3 <?= $materiTersedia ? 'mt-5' : '' ?> mb-3">Daftar Buku PDF</h2>
      <div class="row g-4">
          ...
          <div class="col-lg-4 col-md-6">
            <article class="public-card book-pdf-card h-100">
              <div class="card-body text-center d-flex flex-column">
                ...
                <h3 class="public-title fs-4"><?= htmlspecialchars($data['judul']) ?></h3>
                <p class="file-meta mb-3"><?= number_format((int) $data['ukuran_file'] / 1024, 2) ?> KB<br><i class="bi bi-calendar3"></i> <?= date('d M Y', strtotime($data['created_at'])) ?></p>
                <a href="baca.php?id=<?= (int) $data['id_buku'] ?>" class="public-primary mt-auto"><i class="bi bi-eye me-1"></i>Baca PDF</a>
              </div>
            </article>
          </div>
        <?php endwhile; ?>
      </div>
    <?php elseif (!$materiTersedia): ?>
      <div class="content-card text-center p-5">
        <i class="bi bi-file-earmark-x fs-1 text-danger"></i>
        <h2 class="public-title mt-3">Belum Ada Buku PDF</h2>
        <p class="public-description">Belum ada dokumen buku saku yang diunggah.</p>
      </div>
    <?php endif; ?>
  </div>
</main>
