...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($idSatpam <= 0 || $idShift <= 0 || !$tanggal || !in_array($status, ['bertugas', 'libur'], true)) {
        ...
        if (mysqli_num_rows(mysqli_stmt_get_result($check)) > 0) {
            ...
            if (mysqli_stmt_execute($stmt)) { header('Location: index.php?success=1'); exit; }
...
include "../../includes/header.php"; include "../../includes/admin_navbar.php"; include "../../includes/admin_sidebar.php";
?>
<main class="main-content"><div class="container-fluid"><div class="d-flex justify-content-between align-items-center mb-4"><div><h2 class="fw-bold mb-1">Tambah Jadwal Satpam</h2><p class="text-muted mb-0">Tetapkan petugas dan shift untuk satu hari kerja.</p></div><a href="index.php" class="btn btn-outline-secondary">Kembali</a></div>
<div class="card shadow-sm border-0"><div class="card-body p-4"><form method="post" class="row g-3">
<?php if ($error): ?><div class="col-12"><div class="alert alert-danger mb-0"><?= htmlspecialchars($error) ?></div></div><?php endif; ?>
