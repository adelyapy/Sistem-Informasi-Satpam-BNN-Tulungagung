...
$laporanStmt = mysqli_prepare($conn, "
    ...
    WHERE a.id_satpam = ?
      AND (
          l.status <> 'draft'
          OR EXISTS (SELECT 1 FROM inventaris i WHERE i.id_laporan = l.id_laporan)
          OR EXISTS (SELECT 1 FROM uraian_kegiatan u WHERE u.id_laporan = l.id_laporan)
      )
      ...
      <div class="card-body">
        ...
        <div class="inventory-table table-responsive mobile-scroll-table report-list-table" tabindex="0" aria-label="Tabel daftar laporan, geser ke samping untuk melihat semua kolom">
          <table class="table align-middle report-table">
            ...
            <tbody>
              <?php if (!$laporanTampil) { ?><tr>
                  <td colspan="6" class="text-center text-muted py-5">Tidak ada laporan yang sesuai dengan filter.</td>
                </tr><?php } ?>
              <?php foreach ($laporanTampil as $index => $row) {
                $tervalidasi = $row['status'] === 'tervalidasi';
                $draft = $row['status'] === 'draft'; ?>
                <tr>
                  <td><?= (($halaman - 1) * $perHalaman) + $index + 1 ?></td>
                  <td>
                    <div><?= $formatTanggal($row['tanggal_laporan']) ?></div><small class="text-muted"><i class="bi bi-clock me-1"></i><?= htmlspecialchars(substr($row['created_at'], 11, 5)) ?> WIB</small>
                  </td>
                  <td><?= htmlspecialchars($row['dibuat_oleh'] ?: 'Satpam') ?></td>
                  <td>
                    <div class="fw-medium"><?= htmlspecialchars($row['nama_shift']) ?></div><small class="text-muted"><?= substr($row['jam_mulai'], 0, 5) ?>–<?= substr($row['jam_selesai'], 0, 5) ?></small>
                  </td>
                  <td><span class="report-status <?= $tervalidasi ? 'report-status-valid' : ($draft ? 'report-status-draft' : 'report-status-pending') ?>"><i class="bi <?= $tervalidasi ? 'bi-check-circle-fill' : ($draft ? 'bi-pencil-square' : 'bi-clock-history') ?> me-1"></i><?= $tervalidasi ? 'Sudah divalidasi Kepala BNN' : ($draft ? 'Draft - belum difinalisasi' : 'Menunggu validasi Kepala BNN') ?></span></td>
